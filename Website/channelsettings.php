...
if (isset ( $_POST ['channelName'] )) {
	if ($_FILES['thumbnail']['error'] == UPLOAD_ERR_OK){
		...
		$temp = $newfilename."?cache=".uniqid();
		$stmt2 = mysqli_prepare($conn, "UPDATE channel SET thumbnail=? WHERE username=?");
		mysqli_stmt_bind_param($stmt2, 'ss', $temp, $_SESSION['username']);
		$row["thumbnail"] = $temp;
	...
	$_POST['channelName'] = substr($_POST['channelName'], 0, 255);
	$_POST['channelDescription'] = substr($_POST['channelDescription'], 0, PHP_INT_MAX);
	$_POST['channelName'] = sanitizeHTML($_POST['channelName']);
	$_POST['channelDescription'] = sanitizeHTML($_POST['channelDescription']);
	
	$stmt1 = mysqli_prepare($conn, "UPDATE channel SET name=?, description=?, enabled=? WHERE username=?");
	mysqli_stmt_bind_param($stmt1, 'ssis', $_POST ['channelName'], $_POST ['channelDescription'], $_POST ['enable'], $_SESSION['username']);
	
	if ($uploadOk == 1 && mysqli_stmt_execute($stmt1) === TRUE) {
		echo '<div class="alert alert-success" role="success"><center>Settings saved.</center></div>';
		$row['name'] = $_POST['channelName'];
		$row['description'] = $_POST['channelDescription'];
		$row['enabled'] = $_POST['enable'];
	} else {
	...
	<div class="row">
		...
		<br>
			...
			<div class="form-group">
			  	...
			  	<input class="form-control" id="channelName" name="channelName" type="text" required="" maxlength="255" value="<?php echo $row["name"];?>">
			</div>
			
			<!-- Textarea -->
			<div class="form-group">
				<label for="channelDescription">Channel Description</label>             
				<textarea class="form-control" id="channelDescription" name="channelDescription" rows="11" maxlength=PHP_INT_MAX><?php echo $row["description"];?></textarea>
